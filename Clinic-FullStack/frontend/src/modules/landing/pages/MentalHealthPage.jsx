...
import {
  ...
  MHTelehealthMockup,
  WorkflowCard,
} from '../components/MentalHealthMockups';
import {
  CHECK_COLOR,
  MH_CLINIC_LOGOS,
  MH_FAQ_ITEMS,
  ...
  MH_TRUST_QUOTE,
  MH_WORKFLOW_CARDS,
} from '../data/mental-health-data';
import { cn } from '@/lib/cn';

const MOCKUPS = {
  charting: MHChartingMockup,
  booking: MHBookingMockup,
  intake: MHIntakeMockup,
  billing: CheckoutMockup,
  telehealth: MHTelehealthMockup,
};
    ...
    <SectionShell className="border-y border-[#eeeeee] bg-white py-10 md:py-12">
      <FadeIn className="text-center">
        <h2 className="mb-6 font-heading text-[1.25rem] font-bold text-[#1a1a1a] md:text-[1.5rem]">
          Helping mental health practitioners amplify their impact
        </h2>
        <div className="mx-auto mb-8 flex max-w-3xl flex-col items-center gap-3 sm:flex-row sm:justify-center">
          ...
          <p className="text-body text-[#333333] md:text-body-lg">
            &ldquo;{MH_TRUST_QUOTE.quote}&rdquo; —{' '}
            <span className="font-semibold text-[#1a1a1a]">
              {MH_TRUST_QUOTE.author}, {MH_TRUST_QUOTE.role}
            </span>
          </p>
        </div>
        <div className="flex flex-wrap items-center justify-center gap-6 md:gap-10">
          {MH_CLINIC_LOGOS.map((name) => (
            <span
              key={name}
      ...
      <main>
          ...
          <div className="grid items-center gap-10 lg:grid-cols-[1.15fr_0.85fr] lg:gap-12 xl:gap-16">
            ...
            <FadeIn delay={0.08} className="order-1 lg:order-2 lg:pl-4 xl:pl-8">
              ...
              <h1 className="mb-5 text-balance font-heading text-[2rem] font-bold leading-[1.12] tracking-[-0.02em] text-[#1a1a1a] md:text-[2.5rem] lg:text-[2.75rem]">
                Run your mental health practice with confidence
              </h1>
              <p className="mb-8 max-w-lg text-body-lg leading-relaxed text-[#666666]">
                Tendo is clinic management software that delivers all the tools, privacy, and security
                you need to stay organized and support your patients.
              </p>
        ...
        <SectionShell className="border-b border-[#eeeeee] bg-white py-12 md:py-14">
          <FadeIn className="text-center">
            <h2 className="mx-auto mb-4 max-w-3xl font-heading text-[1.65rem] font-bold text-[#1a1a1a] md:text-[2rem]">
              {MH_SECTION_INTRO.title}
            </h2>
            <p className="mx-auto max-w-2xl text-body-lg text-[#666666]">{MH_SECTION_INTRO.subtitle}</p>
          </FadeIn>
        </SectionShell>

        <SectionShell className="space-y-20 bg-white py-14 md:space-y-28 md:py-20 lg:py-24">
          <FeatureBlock rows={MH_FEATURE_ROWS.slice(0, 3)} />
        </SectionShell>

        <SectionShell className="space-y-20 bg-white py-14 md:space-y-28 md:py-20 lg:py-24">
          <FeatureBlock rows={MH_FEATURE_ROWS.slice(3)} startIndex={3} />
        </SectionShell>
        ...
        <section className="relative overflow-hidden py-16 md:py-24">
          <div
            ...
            style={{
              backgroundImage:
                'linear-gradient(rgba(30,58,95,0.75), rgba(30,58,95,0.85)), url(/images/mental-health/mh-bg-reference.png)',
              backgroundColor: '#1e3a5f',
            }}
          />
          <div className="section-inner relative z-10">
            <div className="grid gap-5 sm:grid-cols-2 lg:gap-6">
              {MH_WORKFLOW_CARDS.map((card, i) => (
        ...
        <SectionShell className="bg-white py-14 md:py-20">
          <FadeIn className="text-center">
            <h2 className="mx-auto mb-10 max-w-3xl font-heading text-[1.35rem] font-bold leading-snug text-[#1a1a1a] md:text-[1.65rem]">
              Why mental health practitioners choose Tendo to grow their practice 🧡
            </h2>
            <div className="mx-auto max-w-[820px] rounded-2xl bg-baby-blue px-8 py-10 md:px-12 md:py-12">
              <div className="mb-6 flex items-center justify-center gap-2">
                ...
                <span className="text-body font-bold text-[#00a3ad]">{MH_TESTIMONIAL.rating}</span>
                <Stars />
              </div>
              <blockquote className="mb-8 text-[1.05rem] font-medium leading-relaxed text-[#1a1a1a] md:text-[1.125rem]">
                &ldquo;{MH_TESTIMONIAL.quote}&rdquo;
              </blockquote>
              <div className="flex items-center justify-center gap-3">
                <div className="flex h-12 w-12 items-center justify-center rounded-full bg-[#00a3ad] text-sm font-bold text-white">
                  {MH_TESTIMONIAL.initials}
                </div>
                <div className="text-left">
                  <p className="text-body-sm font-semibold text-[#1a1a1a]">{MH_TESTIMONIAL.name}</p>
                  <p className="text-caption text-[#666666]">{MH_TESTIMONIAL.role}</p>
                </div>
              </div>
            </div>
          </FadeIn>
        ...
        <SectionShell className="border-t border-[#eeeeee] bg-[#f5f5f5] py-14 md:py-20 lg:py-24">
          <FadeIn className="mb-10 text-center md:mb-14">
            <h2 className="mx-auto mb-4 max-w-3xl font-heading text-[1.65rem] font-bold text-[#1a1a1a] md:text-[2rem]">
              {MH_POWER_SECTION.title}
            </h2>
            <p className="mx-auto max-w-2xl text-body-lg text-[#666666]">{MH_POWER_SECTION.subtitle}</p>
          </FadeIn>
          <FadeIn className="grid items-center gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:gap-14 xl:gap-20">
            <LaptopScheduleMockup />
            <ul className="space-y-4">
              {MH_POWER_FEATURES.map((feature) => (
        ...
        <SectionShell className="bg-primary py-14 md:py-16 lg:py-20" innerClassName="text-center">
          <FadeIn>
            <h2 className="mx-auto mb-8 max-w-[720px] font-heading text-[1.75rem] font-bold leading-snug text-white md:text-[2rem]">
              See the impact simple and flexible tools can have on your workday.
            </h2>
            <Link to={ROUTES.auth.register}>
              <Button
                variant="secondary"
